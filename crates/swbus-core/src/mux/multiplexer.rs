...
impl SwbusMultiplexer {
    ...
    #[instrument(name = "update_route", level = "info", skip(self, nexthop))]
    pub(crate) fn update_route(&self, route_key: String, nexthop: SwbusNextHop) {
        ...
        info!(?nexthop.nh_type, nexthop.hop_count, conn_info=nexthop.conn_info.as_ref().map(|x| x.id()).unwrap_or(&"None".to_string()), "Updating route to nexthop");
